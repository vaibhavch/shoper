...
def pre_verify(request):
    ...
    isalready = UserInfo.objects.filter(mobile=uphone, mobile__isnull=False)
    if isalready:
       userid = UserInfo.objects.get(mobile=uphone)
       if userid.otpstatus == 0:
           headers = { 'Content-Type': 'application/json' }
           values = {'method' : 'sms',
                 'api_key' : 'Aa0bcb8adeb7b52c3ae8dd9d3ec16b64a',
                 'to' : uphone,
                 'sender' : 'SHOPER',
                 'message' : 'Your Shoper verification code is '+userid.otp+' Enjoy :)',
                 'format' : 'json',
                 'flash' : '0',
                 }
...
def userinfo(request):
    currentuser = UserInfo.objects.get(user = request.user)
    if request.user.is_authenticated():
        return Response(currentuser.mobile)
...
def staff_login(request):
     ...
     if user is not None:
         return Response(Token.objects.get(user=user).key)
     else:
         return Response("not")
...
@api_view(['GET'])
def time(request):
    return Response(timezone.localtime(timezone.now()).hour)


def pdf(request):
    orderid = request.GET.get('id','')
    order = Orders.objects.get(id=orderid)
    currentuser = Orders.objects.get(id=orderid).user
    userinfo = UserInfo.objects.get(user = currentuser.id)

    address = Orders.objects.get(id=orderid).address

    productlistn = []
    productids = Orders.objects.get(id=orderid).productidarray.replace('[', '').replace(']','')
    productidsl = [int(x) for x in productids.split(',')]
    for v in productidsl:
       productlistn.append(v)

    if order.type == 'GR':
       productlist = Products.objects.filter(id__in=productlistn)
    else:
       productlist = Foods.objects.filter(id__in=productlistn)

    listobjects = dict([(obj.id, obj) for obj in productlist])
    sorted_objects = [listobjects[id] for id in productlistn]

    qtyids = Orders.objects.get(id=orderid).quantityidarray.replace('[', '').replace(']','')
    qtyarray = []
    qtysplit = [int(x) for x in qtyids.split(',')]
    for v in qtysplit:
       qtyarray.append(v)
    productlistnqty = zip(sorted_objects, qtyarray)
   

    html_template = get_template('invoice.html')
    rendered_html = html_template.render(RequestContext(request, {'order': order, 'userinfo': userinfo, 'address': address, 'productlist': productlistnqty })).encode(encoding="UTF-8")
